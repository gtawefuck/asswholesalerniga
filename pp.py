...
SHOP_URL = "https://shawniandbooboolux.myshopify.com"
VARIANT_ID = None

CHECKOUT_DATA = {
    "email": "mail45435@gmail.com",
    "first_name": "John",
    "last_name": "Doe",
...
CARD_DATA = {
    "number": "5328397239087806",
    "month": 4,
    "year": 2029,
    "verification_value": "151",
    "name": "John Doe   "
}
    ...
    while site_attempt < len(sites) and not worked:
        ...
        while attempts < max_attempts and not worked and not site_skipped:
            ...
            with PRODUCT_CACHE_LOCK:
                cached = site_product_cache.get(shop_url)
            if cached:
                product_id, variant_id, price_str, title = cached
            else:
                product_id, variant_id, price_str, title = auto_detect_cheapest_product(session, shop_url)
                if variant_id:
                    with PRODUCT_CACHE_LOCK:
                        site_product_cache[shop_url] = (product_id, variant_id, price_str, title)
            if not variant_id:
                print("\n❌ [ERROR] Could not find any products on this site")
                print(f"[INFO] Switching to next proxy (attempt {attempts}/{max_attempts})")
                continue
            ...
            if isinstance(receipt_result, tuple):
                if len(receipt_result) >= 5:
                    receipt_id, submit_code, submit_message, submit_resp, amount_charged = receipt_result
...
def main():
    ...
    print("=" * 70)
    print("Dynamic Shopify Checkout - Single Site Mode")
    print("=" * 70)
    print(f"Target site: {SHOP_URL}")
    print(f"Card: **** **** **** {CARD_DATA.get('number', '')[-4:]}")
    
    attempt_start_time = time.time()
    
    # Create session without proxy
    session = create_session(SHOP_URL, proxies=None)
    
    # Auto-detect cheapest product
    product_id, variant_id, price, title = auto_detect_cheapest_product(session, SHOP_URL)
    if not variant_id:
        print("\n❌ [ERROR] Could not find any products on this site")
        return
